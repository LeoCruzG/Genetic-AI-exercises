...
def evaluation (q):
    ...
    tTom = math.hypot(fxx[0], 1.2)/2
    tDick = math.hypot(fxx[1], 2.5)/4
    tMary = (4.3-fxx[0]-fxx[1])/6
    total = tTom + tDick + tMary
...
x = 0.352479654846
y =  0.6365979476687
tTom = math.hypot(x, 1.2)/2
ttom = (math.sqrt(math.pow(x, 2)+math.pow(1.2, 2)))/2
tDick = math.hypot(y, 2.5)/4
tdick = (math.sqrt(math.pow(y, 2)+math.pow(2.5, 2)))/4
tMary = (4.3-x-y)/6
tot = tTom + tDick + tMary
print (tTom,ttom,  tDick, tdick, tMary, tot)
